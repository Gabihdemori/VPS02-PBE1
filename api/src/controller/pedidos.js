...
const update = (req, res) => {
    ...
    let query = `UPDATE pedidos SET pedido_id = '${idcliente}', idprod = '${idprod}', idtelefone = '${idtelefone}', datalancamento = '${datalancamento}', total = '${total}' WHERE pedidos_id = ${req.params.id} `
    con.query(query, (err, result) => {
        if (err) {
            res.status(500).json(err)
        } else {
...
const deletar = (req, res) => {
    ...
    const query = 'DELETE FROM pedidos WHERE pedidos_id = ?';
